...
class Broker (Node):
    ...
    def __init__(self,fovmat,sceneData,manager):
    ...
    def callback_robot_gen(self,msg):
        ...
        sim.setObjectOrientation(robot,-1,o)
            # Crear un robot como un dict gestionado
        robot_data = self.mMan.dict()
        robot_data["Pose"] = self.mMan.list([p[0],p[1],o[2]])
        robot_data["Nickname"] = str(msg.data)
        robot_data["Battery"] = 100
        robot_data["Health"] = 100
        robot_data["Skill"]=self.mMan.dict({"Hammer": False,"Shield":False})
        
        self.sceneData["Robots"][alias]=robot_data
        #self.write()                            
        '''
        {
        "Robots": {
        ...
        #MULTIPROCESSING
        proc=multiprocessing.Process(target=robot_thread,args=("Robot_"+str(self.n_robots),self.fovmat,self.sceneData)) #argumento es el handler
        proc.start()
    ...
    def initialize(self):
        ...
        initial_data = json.loads(sim.getBufferProperty(sim.handle_scene, "customData.myTag"))
        #print(initial_data)
        self.sceneData["Obstacles"] = initial_data["Obstacles"]
        self.sceneData["Chargers"] = initial_data["Chargers"]
        
        self.sceneData["Skills"]=self.mMan.dict()
        
        for sk in initial_data["Skills"]:
            skill_entry = self.mMan.dict()
            pos_list = self.mMan.list(initial_data["Skills"][sk]["Position"])  # Asigna directamente la lista

            skill_entry["Position"] = pos_list
            skill_entry["Skill"] = initial_data["Skills"][sk]["ID"]
            skill_entry["Enabled"] = initial_data["Skills"][sk]["Enabled"]

            self.sceneData["Skills"][sk] = skill_entry
    ...
    def get_all_positions(self):
        ...
        for categoria in ["Robots", "Chargers", "Skills", "Obstacles"]:
            contenedor = self.sceneData.get(categoria)
            if contenedor is None:
                continue

            # Para DictProxy: asegúrate de acceder a .values() correctamente
            for objeto in contenedor.values():
                try:
                    if categoria == "Robots" and "Pose" in objeto:
                        posiciones.append(list(objeto["Pose"])[:2])
                    elif "Position" in objeto:
                        posiciones.append(list(objeto["Position"])[:2])
                except Exception as e:
                    print(f"[WARN] Error accediendo a {categoria}: {e}")
                    continue
    ...
    def enable_used_skills(self):
        if verbose:
            print(f"[ DEBUG BROKER ] Enabling skills")
        skills=dict(self.sceneData["Skills"])
        for sk in skills:
            if not self.sceneData["Skills"][sk].get("Enabled"):
                self.sceneData["Skills"][sk]["Enabled"]=True
                skill_handle=sim.getObject(f"/Scene/{sk}")
                p = np.random.uniform(-6, 6, size=2)
                poses = self.get_all_positions()
                while (not self.valid_pose(p[:2],poses)):
                    p = np.random.uniform(-6, 6, size=2)

                sim.setObjectPosition(skill_handle,-1,[p[0],p[1],0.05])
    ...
    def calculateFOV(self): # Aqui no se si se estan actualizando
        if self.sceneData:
            ...
            calc = multiprocessing.Process(target=Fov_array, args=(self.sceneData,self.fovmat))
            calc.start()
...
def Fov_array(sceneData,fovmat):
    ...
    def get_positions(contenedor):
        ...
        # Soporte tanto para dict normales como DictProxy
        try:
            for key in contenedor.keys():
                obj = contenedor[key]
                if "Pose" in obj:
                    elementos.append(obj["Pose"][:2])
                elif "Position" in obj:
                    elementos.append(obj["Position"][:2])
        except Exception as e:
            print(f"[get_positions] Error accediendo a elementos: {e}")
            return np.empty((0, 2))
    ...
    fovcopy={}
    for i, robot in enumerate(sceneData["Robots"].keys()):
        alias = robot
        pos = robots_pos[i]
        fovcopy[alias] = {}

        # Distancias a otros robots
        dist_robs = []
        for j, other_pos in enumerate(robots_pos):
            d = -1 if i == j else np.linalg.norm(pos - other_pos)
            dist_robs.append(d)
        ...
        for name, positions in [("chargers", chargers_pos), ("skills", skills_pos), ("obstacles", obstacles_pos)]:
            dist_list = [np.linalg.norm(pos - obj_pos) for obj_pos in positions]
...
def robot_thread(name,fovmat,sceneData):
    rclpy.init()
    class RobotNode(Node):
        def __init__(self,fovmat,sceneData):
            ...
            self.last_fovmat=self.fovmat.get(self.alias,{}).get('skills')
        ...
        def thread(self):
            if not self.endgame:
                ...
                self.sceneData["Robots"][self.alias]["Pose"][0]=p[0]
                self.sceneData["Robots"][self.alias]["Pose"][1]=p[1]
                self.sceneData["Robots"][self.alias]["Pose"][2]= o[2]

                # Battery management

                self.battery = self.sceneData["Robots"][self.alias]["Battery"]
                self.close2charger= False 
                for dist in (self.fovmat.get(self.alias,{}).get("chargers") or []):
                    if dist < 0.2:
                ...
                close2skill=False
                if self.last_fovmat !=self.fovmat.get(self.alias,{}).get('skills'):
                    self.last_fovmat=self.fovmat.get(self.alias,{}).get('skills')
                    for index,skill_pos in enumerate(self.fovmat.get(self.alias,{}).get('skills')):
                        if skill_pos<0.5:
                            name = "skill_"+str(index+1)
                            close2skill=True if self.sceneData["Skills"][name]["Enabled"] else False
                            break
                        
                # Identificar la skill
                if close2skill:
                    print("Close to any skill")
                    ability=self.sceneData["Skills"][name].get("ID")
                    # Invisibilizar bloque de habilidad 
                    sim.setObjectInt32Param(sim.getObject(f"/Scene/{name}"), sim.objintparam_visibility_layer, 1)
                    sim.setObjectPosition(sim.getObject(f"/Scene/{name}"),-1,[0.0,0.0,-6.0])
                    self.sceneData["Skills"][name]["Enabled"]=False
                ...
                if self.shieldEnabled:
                    if (sim.getSimulationTime()-self.shieldStartTime)>self.ability_duration:
                ...
                if self.hammerEnabled:
                    ...
                    if not self.hammerUsed and active_time<self.ability_duration:
                        ...
                        self.close2oponent=False
                        for i,distance in enumerate(self.fovmat.get(self.alias,{}).get("robots") or []):
        ...
        def write(self):
            self.sData.update(self.sceneData)
        def debugging(self):
            if self.verbose:
                if not self.endgame:
        ...
        def pub_laser(self):
            ...
            pass
    node = RobotNode(fovmat,sceneData)
...
def main(args=None):
    ...
    sceneData = manager.dict()
    
    sceneData["Robots"] = manager.dict()


    
    rclpy.init(args=args)
    node = Broker(fovmat,sceneData,manager)
    while rclpy.ok():
        try:
            rclpy.spin(node)
        except KeyboardInterrupt:
